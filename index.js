...
var React, {Platform, NativeModules, NativeEventEmitter, NativeAppEventEmitter, DeviceEventEmitter, Platform} = require('react-native');

var AudioPlayerManager = NativeModules.AudioPlayerManager;
var AudioRecorderManager = NativeModules.AudioRecorderManager;
var RecordEmitter = new NativeEventEmitter(AudioRecorderManager);
...
var AudioRecorder = {
  prepareRecordingAtPath: function(path, options) {
    ...
    if (Platform.OS === 'ios') {
      if (this.finishedSubscription) this.finishedSubscription.remove();
      this.finishedSubscription = RecordEmitter.addListener('recordingFinished', (data) => {
        console.warn('onFinish??')
        console.warn('FINISHED', JSON.stringify(data,null,2));
        if (this.onFinished) {
          this.onFinished(data);
        }
      });

      // if (this.progressingSubscription) this.progressingSubscription.remove();
      // this.progressingSubscription = RecordEmitter.addListener('recordingProgress', (data) => {
      //   console.warn('PROGRESS', JSON.stringify(data,null,2));
      // });
    }
  },
